...
const requiredEnv = [
  ...
  'MICROSOFT_RH_GROUP_ID',
  'MICROSOFT_MANAGER_GROUP_ID',
  'MICROSOFT_DIRECTOR_GROUP_ID'
];
...
pdf
  ...
  .text(
    row.doc_type === 'restitution'
      ? "FICHE DE RESTITUTION DE MATÉRIEL INFORMATIQUE"
      : isReassignment
...
pdf.text(
  `${isReassignment ? 'Date de mise à jour' : 'Date de signature'} : ${formatDateFr(
...
async function runScheduledMicrosoftLicenseSync() {
  if (microsoftLicenseSyncRunning) {
    console.log('[LICENSE SYNC] Synchronisation précédente encore en cours, exécution ignorée');
    return;
  }

  microsoftLicenseSyncRunning = true;

  try {
    console.log('[LICENSE SYNC] Début de la synchronisation automatique');
    ...
    const res = {
      ...
      json(payload) {
        if (this.statusCode >= 400) {
          console.error('[LICENSE SYNC] Échec :', payload);
        } else {
          console.log('[LICENSE SYNC] Résultat :', payload);
        }
        return payload;
      }
    };

    await syncMicrosoftLicenses(req, res);
  } catch (error) {
    console.error('[LICENSE SYNC] Erreur inattendue :', error);
  } finally {
    microsoftLicenseSyncRunning = false;
  }
}
...
app.get('/api/hardware-groups', async (req, res) => {
  const result = await pool.query(`
    SELECT *
...
app.post('/api/hardware-assignments/user', async (req, res) => {
  ...
  await pool.query(
    `
    INSERT INTO hardware_assignments
    (
      hardware_item_id,
      employee_id
    )
...
app.post('/api/hardware-assignments/group', async (req, res) => {
...
app.post('/api/hardware-assignments/unassign', async (req, res) => {
...
const infoResult = await pool.query(
  ...
  LEFT JOIN onboarding_details od
    ON od.movement_id = m.id
  WHERE m.id = $1
  `,
  [id]
);
...
console.log('NOT_HIRED INFO', info);

      await pool.query(
        `
        UPDATE movements
        SET status = 'non_embauche'
        WHERE id = $1
        `,
        [id]
      );
...
await sendMailWithAttachments({
  ...
  subject: `Annulation Onboarding - ${info.first_name} ${info.last_name}`,
  html: `
    <p>La demande d'onboarding de
    <strong>${info.first_name} ${info.last_name}</strong>
    a été annulée par le service RH.</p>
  `,
  attachments: []
});
...
if (info.company_car) {
  ...
  await sendMailWithAttachments({
    ...
    html: `
      <p>
        La demande d'onboarding de
        <strong>${info.first_name} ${info.last_name}</strong>
        a été annulée par le service RH.
      </p>

      <p>
        Une voiture de fonction avait été demandée.
      </p>
    `,
    attachments: []
  });
...
groupIds: [
  process.env.MICROSOFT_IT_GROUP_ID,
  process.env.MICROSOFT_RH_GROUP_ID,
  process.env.MICROSOFT_MANAGER_GROUP_ID,
  process.env.MICROSOFT_DIRECTOR_GROUP_ID
]
...
const isIT = allowedGroups.includes(
  process.env.MICROSOFT_IT_GROUP_ID
);

const isRH = allowedGroups.includes(
  process.env.MICROSOFT_RH_GROUP_ID
);

const isManager = allowedGroups.includes(
  process.env.MICROSOFT_MANAGER_GROUP_ID
);

const isDirector = allowedGroups.includes(
  process.env.MICROSOFT_DIRECTOR_GROUP_ID
);
...
app.get('/api/my-onboarding-requests', async (req, res) => {
  try {
const userEmail = req.session.user?.email;
const isRH = req.session.user?.isRH;
...
await sendMailWithAttachments({
  ...
  subject:
    `Demande onboarding - ${row.first_name} ${row.last_name}`,

  html: `
    <p>Bonjour,</p>

    <p>
      Cette demande d'onboarding vous est renvoyée.
    </p>
...
const rawUserSource =
  typeof row.import_user_source === 'string'
    ? row.import_user_source.trim()
    : '';

const rawUserLabel =
  typeof row.import_user_label === 'string'
    ? row.import_user_label.trim()
    : '';

let normalizedEmail = null;

const emailMatch = rawUserSource.match(
...
if (emailMatch) {
  ...
  if (
    normalizedEmail.endsWith('@elyade')
  ) {
    normalizedEmail += '.com';
  }
}
...
const looksLikeGroup =
  rawUserLabel.startsWith('🏢') ||
  /(?:^|\s)-?\s*membres$/i.test(
    rawUserLabel
  ) ||
  rawUserLabel.toLowerCase().includes(
    'pole '
  ) ||
  rawUserLabel.toLowerCase().includes(
    'service '
...
if (normalizedEmail) {
  const employeeResult = await pool.query(
    ...
    WHERE
      lower(email) = lower($1)
      OR lower(microsoft_upn) = lower($1)
    LIMIT 1
    `,
    [normalizedEmail]
  );
...
let importAssignmentNote = '';

if (
  !employeeId &&
  !looksLikeGroup &&
  (rawUserSource || rawUserLabel)
) {
  importAssignmentNote =
    `Affectation importée non résolue : ` +
    `Usager="${rawUserSource || '-'}" ; ` +
    `Utilisateur="${rawUserLabel || '-'}"`;
}

if (importAssignmentNote) {
...
LIMIT 1
  ...
  if (assignmentResult.rows.length > 0) {
    ...
    return res.status(400).json({
      error:
        'Impossible de mettre en stock un matériel affecté à un collaborateur'
    });
...
app.post('/api/assignments/group', async (req, res) => {
  try {
...
if (
  ...
  return res.status(400).json({
    error: 'Type de bénéficiaire invalide.'
  });
}
...
if (
  ...
  return res.status(400).json({
    error: 'Collaborateur obligatoire.'
  });
}
...
if (!Array.isArray(changes) || changes.length === 0) {
  ...
  return res.status(400).json({
    error: 'Aucun changement transmis.'
  });
}
...
let createdSignedDocument = null;
...
createdSignedDocument = signedDocumentResult.rows[0];
}
    await client.query('COMMIT');
if (createdSignedDocument) {
    try {
...
app.get('/api/inventory/preferences/:email', async (req, res) => {
  try {
...
app.post('/api/inventory/preferences', async (req, res) => {
  try {
    const {
      email,
      visibleColumns,
    ...
    await pool.query(
      `
      INSERT INTO user_inventory_preferences
      (
        user_email,
        visible_columns,
        column_order,
...
setInterval(async () => {
  try {
    console.log('MICROSOFT USERS SYNC START');
    ...
    await syncMicrosoftUsers(
      {},
      {
        json: (result) =>
          console.log(
            'MICROSOFT USERS SYNC RESULT',
            result
          ),
        status: () => ({
          json: (err) =>
            console.error(
              'MICROSOFT USERS SYNC ERROR',
              err
            )
        })
      }
